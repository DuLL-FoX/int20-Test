...
export async function POST(req: NextRequest) {
    ...
    if (lot.userId === userId) return respondWithError("You can't bid on your own lot.", 400);
    if (lot.startPrice >= bidAmount) return respondWithError("Bid amount should be greater than the start price.", 400);

    const bid = await db.auctionBid.create({ data: { lotId: lot.id, userId, bidAmount } });
